...
* **Portail Super Admin (Créateur de la plateforme)** :
  * Accessible directement via l'URL dédiée : **`/super-admin`** (ou via le bouton *Portail Super Admin* dans le panneau de profil du compte `boss`).
  * Affiche les statistiques globales de la plateforme :
    * 👔 Nombre de Boss (Entreprises / Managers) et 👷 d'Employés.
    * 📁 Projets au total (en cours / archivés).
    * 📋 Rapports journaliers soumis et commentaires associés.
    * 💬 Messages échangés et 📎 fichiers téléversés.
    * 📋 Registre complet de tous les comptes utilisateurs inscrits.
    * 💥 Bouton rapide de remise à zéro de la plateforme.
* **Créer un Projet** :
